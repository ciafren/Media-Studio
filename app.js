...
const defaults={preset:'auto',strength:70,exposure:0,contrast:0,highlights:0,shadows:0,temperature:0,saturation:0,vibrance:0,clarity:0,sharpness:0,vignette:0};
const presets={
...
const keys=['strength','exposure','contrast','highlights','shadows','temperature','saturation','vibrance','clarity','sharpness','vignette'];
...
function syncCanvasLayout(){requestAnimationFrame(()=>{const stage=$('#imageViewer'),o=$('#originalCanvas'),e=$('#editedCanvas');const ratio=o.width/o.height;const sw=stage.clientWidth,sh=stage.clientHeight;let w=sw,h=w/ratio;if(h>sh){h=sh;w=h*ratio}o.style.width=e.style.width=w+'px';o.style.height=e.style.height=h+'px';e.style.left=((sw-w)/2)+'px';o.style.left=((sw-w)/2)+'px';o.style.top=e.style.top=((sh-h)/2)+'px';const clip=$('#editedClip');clip.style.left=((sw-w)/2)+'px';clip.style.top=((sh-h)/2)+'px';clip.style.height=h+'px';clip.style.width=(w*($('#compareRange').value/100))+'px';$('#compareLine').style.left=((sw-w)/2+w*($('#compareRange').value/100))+'px';$('#compareLine').style.top=((sh-h)/2)+'px';$('#compareLine').style.height=h+'px'})}
...
function renderToCanvas(source,canvas,settings){
 ...
 const exp=Math.pow(2,(s.exposure||0)/100*1.5);const con=(s.contrast||0)/100;const sat=(s.saturation||0)/100;const vib=(s.vibrance||0)/100;const temp=(s.temperature||0)/100;const hi=(s.highlights||0)/100;const sh=(s.shadows||0)/100;const clarity=(s.clarity||0)/100;
...
function loadSettingsUI(s){s={...defaults,...s};keys.forEach(k=>{const el=$('#'+k);if(el)el.value=s[k]??defaults[k];const out=$('#'+k+'Out');if(out)out.value=k==='strength'?`${s[k]}%`:(s[k]>0?`+${s[k]}`:`${s[k]}`)});$$('.preset').forEach(b=>b.classList.toggle('active',b.dataset.preset===s.preset))}
async function updateSetting(key,val){if(!currentMedia)return;currentMedia.settings={...defaults,...currentMedia.settings,[key]:Number(val)};await put('media',currentMedia);const out=$('#'+key+'Out');if(out)out.value=key==='strength'?`${val}%`:(Number(val)>0?`+${val}`:`${val}`);scheduleRender()}
...
function setView(mode){['compare','edited','original'].forEach(m=>$('#'+m+'Btn').classList.toggle('active',m===mode));const clip=$('#editedClip'),line=$('#compareLine'),range=$('#compareRange');if(mode==='compare'){clip.style.display='block';line.style.display='block';range.style.display='block';$('#originalCanvas').style.display='block'}else if(mode==='edited'){clip.style.display='block';clip.style.width=$('#originalCanvas').style.width;line.style.display='none';range.style.display='none';$('#originalCanvas').style.display='none'}else{clip.style.display='none';line.style.display='none';range.style.display='none';$('#originalCanvas').style.display='block'}syncCanvasLayout()}
